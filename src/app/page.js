...
export default function HomePage() {
  ...
  const onNavigateCatalog = (category) => {
    const query = category ? `?category=${category}` : "";
    window.location.href = `/catalog${query}`;
  };

  const onNavigateProduct = (productId) => {
  ...
  return (
    <div className="bg-[#F8F7FF]">
      <section className="bg-[#5B50E8] relative overflow-hidden">
        <div
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-white/15 text-white text-sm font-medium px-4 py-2 rounded-full mb-6">
              ...
              Mais de 12.000 itens disponíveis hoje
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-7 gap-3">
          {CATEGORIES.filter((c) => c !== "Todos").map((cat) => (
            <button
              key={cat}
              onClick={() => onNavigateCatalog(cat)}
              className="group flex flex-col items-center gap-2 p-4 rounded-2xl bg-white border border-gray-100 hover:border-[#5B50E8]/30 hover:bg-[#EAE8FD] transition-all"
            >
              <span className="text-2xl">{categoryIcons[cat] ?? "🏷️"}</span>
              <span className="text-xs font-medium text-gray-600 group-hover:text-[#5B50E8] transition-colors text-center leading-tight">
                {cat}
              </span>
            </button>
          ))}
      ...
      <section className="mx-4 sm:mx-6 lg:mx-8 mb-12 max-w-7xl lg:mx-auto lg:px-8">
        <div className="bg-[#1C1B2E] rounded-3xl p-8 lg:p-12 flex flex-col lg:flex-row items-center justify-between gap-8 relative overflow-hidden">
          ...
          <div className="relative flex-shrink-0 grid grid-cols-2 gap-4 w-full lg:w-auto">
            {[
              { icon: "♻️", value: "840t", label: "CO₂ economizado" },
              { icon: "💧", value: "2.3M L", label: "Água poupada" },
              { icon: "👕", value: "180k", label: "Peças reaproveitadas" },
              { icon: "🌳", value: "6.200", label: "Árvores equivalentes" },
            ].map((stat) => (
              <div
                key={stat.label}
      ...
      <footer className="bg-[#1C1B2E] text-white/50 py-8 px-4 sm:px-6 lg:px-8 text-center text-sm">
        <p>© 2026 ReUse. Feito com 💚 para um mundo melhor.</p>
      </footer>
    </div>
  );
}
